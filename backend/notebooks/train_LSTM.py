...
def load_sentiment_data(sentiment_dir: str) -> pd.DataFrame:
    ...
    Returns a DataFrame with Date and sentiment score (prob_positive - prob_negative)
    """
    all_sentiment = []
    ...
    for file_path in sentiment_files:
        try:
            ...
            # Keep only necessary columns
            df_sent = df_sent[['Date', 'Sentiment']].copy()
...
# %%
## Calculate Market Sentiment for Each Day
...
def calculate_market_sentiment(price_df: pd.DataFrame, sentiment_df: pd.DataFrame, lambda_decay: float) -> pd.DataFrame:
    """
    Calculate overall market sentiment for each day in price_df.
    ...
    Returns:
        DataFrame with Date and Market_Sentiment columns
    """
    ...
    for current_date in unique_dates:
        # Get all sentiment events that occurred before this day (not including current day)
        prior_events = sentiment_df[sentiment_df['Date'] < current_date].copy()
        
        if len(prior_events) == 0:
            market_sentiment = 0.0
        else:
            # Calculate days since each event (both are date objects, so subtraction gives timedelta)
            prior_events['Days_Since'] = (pd.to_datetime(current_date) - pd.to_datetime(prior_events['Date'])).dt.days
            
            # Calculate weighted sentiment: sentiment * exp(-lambda * dt)
            prior_events['Weighted_Sentiment'] = (
                prior_events['Sentiment'] * np.exp(-lambda_decay * prior_events['Days_Since'])
            )
            
            # Sum all weighted sentiments
            market_sentiment = prior_events['Weighted_Sentiment'].sum()
...
# Calculate market sentiment for each day
market_sentiment_df = calculate_market_sentiment(df_all, sentiment_df, LAMBDA)
print(f"\nMarket sentiment calculated for {len(market_sentiment_df)} dates")
print(f"Market sentiment statistics:")
print(market_sentiment_df['Market_Sentiment'].describe())
print("\nFirst few rows:")
print(market_sentiment_df.head(10))


# %%
## Merge Sentiment with Price Data

# Merge market sentiment with price data
df_all['Date'] = pd.to_datetime(df_all['Date']).dt.date
df_combined = df_all.merge(market_sentiment_df, on='Date', how='left')

# Fill any missing sentiment values with 0 (days with no sentiment events)
df_combined['Market_Sentiment'] = df_combined['Market_Sentiment'].fillna(0.0)
...
print(df_combined.groupby('Ticker')['Market_Sentiment'].agg(['mean', 'std', 'min', 'max']).head(10))
...
# Prepare sequences with sentiment
# Use df_combined which includes Market_Sentiment
X, y = prepare_sequences(df_combined, config.SEQUENCE_LENGTH, feature_cols=['Close_Return', 'Market_Sentiment'])
print(f"\nClose Return statistics:")
print(f"Mean: {y.mean():.4f}, Std: {y.std():.4f}")
print(f"Min: {y.min():.4f}, Max: {y.max():.4f}")
print(f"\nInput features: Close_Return and Market_Sentiment")
...
class LSTMSentiment(nn.Module):
    """
    LSTM model for time series forecasting with sentiment features.
    Takes both price features (Close_Return) and market sentiment as input.
    """
    def __init__(
        self,
        input_size: int,
    ...
    def forward(self, x):
        ...
        Args:
            x: Input tensor of shape (batch_size, sequence_length, input_size)
               where input_size includes both Close_Return and Market_Sentiment

        Returns:
            output: Tensor of shape (batch_size,) - predicted Close_Return for next day
        """
        # LSTM forward pass
        lstm_out, (h_n, c_n) = self.lstm(x)
...
# Initialize model with sentiment
n_features = X_train.shape[2]  # Should be 2: Close_Return and Market_Sentiment
print(f"Number of input features: {n_features} (expected: 2 for Close_Return and Market_Sentiment)")
